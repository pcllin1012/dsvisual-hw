...
An adjacency matrix represents a graph as a $V\times V$ matrix $M$: if there is an edge between vertices $i$ and $j$, $M[i][j]$ records it (or its weight). Space is $O(V^2)$, and an edge lookup is $O(1)$.

---

## Core Concept

Row $i$ of the matrix describes every edge touching vertex $i$; for an undirected graph $M[i][j] = M[j][i]$, so the matrix is symmetric across its main diagonal.

- Undirected ⇒ a symmetric matrix (the diagonal stays 0 — no self-loops here).
- Weighted graph: each cell stores the weight $w$ instead of a plain 1; 0 still means "no edge".
- Enumerating vertex $i$'s neighbours means scanning its whole row, costing $O(V)$.
- A good fit for dense graphs; the sparser the graph, the more wasted zero cells.

---

## Operation Flow

1. Allocate a $V\times V$ matrix with every cell initialised to 0.
2. To add edge $(u, v, w)$, set `adj[u][v] = w`.
3. If the graph is undirected, also mirror it with `adj[v][u] = w` to keep the matrix symmetric.
4. An edge query for $(i, j)$ is a direct read of `adj[i][j]`; nonzero means an edge exists (the value is its weight).
...
> Blue cells are the edges of the example graph (numbers are their weights); the rest are 0 (no edge). The matrix is symmetric across the diagonal because the graph is undirected.

---

## Worked Example

Input: 5 vertices, undirected graph with edges 0-1, 0-4, 1-2, 1-3, 1-4, 2-3, 3-4 (weights 4, 1, 3, 2, 5, 6, 7 respectively).

1. Initialise a 5×5 matrix of all zeros.
2. Add edge 0-1 (w=4): `adj[0][1] = adj[1][0] = 4`.
3. Add the remaining edges 0-4(1), 1-2(3), 1-3(2), 1-4(5), 2-3(6), 3-4(7) the same way, mirroring each value.
4. The finished row 1 (vertex 1) reads [4, 0, 3, 2, 5] — nonzero at columns 0, 2, 3, 4, so degree(1) = 4.
5. `outDegree(1)` scans that row for nonzero entries and also returns 4, matching the matrix directly.
