...
test.describe('Tree VCR step log', () => {
  ...
  test('tree-fenwick: code drawer + step log wired to the transport', async ({ page }) => {
    await loadMethod(page, 'tree-fenwick');
    const card = page.locator('[data-method-section="tree-fenwick"]');
    // code drawer (newly added)
    await expect(card.locator('[data-testid="code-drawer"]')).toHaveCount(1);
    await expect(card.locator('[data-testid="code-drawer"] .code-panel-filename')).toContainText('tree_fenwick.cpp');
    // workbench + step log
    await expect(card.locator('.viz-workbench')).toBeVisible();
    const log = card.locator('[data-testid="viz-steplog"]');
    await expect(log).toBeVisible();
  ...
  test('tree-trie: code drawer + step log wired to the transport', async ({ page }) => {
    await loadMethod(page, 'tree-trie');
    const card = page.locator('[data-method-section="tree-trie"]');
    // code drawer (already present)
    await expect(card.locator('[data-testid="code-drawer"]')).toHaveCount(1);
    // workbench + step log
    await expect(card.locator('.viz-workbench')).toBeVisible();
    const log = card.locator('[data-testid="viz-steplog"]');
  ...
  test('tree-dsu: code drawer + step log wired to the transport', async ({ page }) => {
